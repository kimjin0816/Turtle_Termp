<template>
  <v-app>
    ...
    <v-app-bar app color="grey lighten-1" dark style="max-height: 63px">
      ...
      <!-- 메뉴 버튼 -->
      <v-row
        style="display: flex; align-items: center; margin-top: 150px !important; justify-content: space-between;"
      >
        <v-btn
...
export default {
  ...
  methods: {
    goToPage(pageName) {
      try {
        if (this.$route.name !== pageName) { // 현재 경로의 이름과 이동하려는 페이지 이름이 같지 않을 때만 이동을 수행
          this.$router.push({
            name: pageName,
