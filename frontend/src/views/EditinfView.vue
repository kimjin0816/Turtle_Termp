...
사용 가능 특수문자 : ~ ․! @ # $ % ^ & * ( ) _ - + =  [ ] [ ] | \ ; :‘ “ < > , . ? /
      ...
      <button @click="updateProfile" class="logon_btn" type="submit">수정하기</button>
    </form>
  </div>
</template>
  ...
  methods: {
    ...
    async getProfile() {
      ...
      if (response.status == 200) {
        ...
        this.contact = response.data.mem_tel;
        this.mem_nickname = response.data.mem_nickname;
        this.mem_address = response.data.mem_address;

        if (response.data.mem_password.length >= 9) {
          this.password_possible = true;
        }
      }
    },
    async updateProfile() {
      if (this.password_possible == true) {
        try {
          ...
          if (this.mem_id == "" || this.mem_password == "") {
